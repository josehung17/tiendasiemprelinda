        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @can('edit categories')
                    <form method="POST" action="{{ route('categorias.update', $categoria->id) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="nombre" :value="__('Nombre')" />
                            <x-text-input id="nombre" name="nombre" type="text" class="mt-1 block w-full" required autofocus value="{{ old('nombre', $categoria->nombre) }}" />
                        ...
                        <div>
                            ...
                            @if ($categoria->ruta_imagen)
                                <img src="{{ asset('storage/' . $categoria->ruta_imagen) }}" alt="Imagen de {{ $categoria->nombre }}" class="mt-2 h-24 w-24 object-cover rounded">
                            @else
                    ...
                    @else
                    <p class="text-red-500 dark:text-red-400">No tienes permiso para editar categorías.</p>
                    @endcan
                </div>
            </div>
        </div>
