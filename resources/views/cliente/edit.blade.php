                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('clientes.update', $cliente->id) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="nombre" name="nombre" type="text" class="mt-1 block w-full" required autofocus value="{{ old('nombre', $cliente->nombre) }}" />
                            <x-input-error class="mt-2" :messages="$errors->get('nombre')" />
                        </div>

                        <div>
                            <x-input-label for="numero_documento" :value="__('Número de Documento')" />
                            <x-text-input id="numero_documento" name="numero_documento" type="text" class="mt-1 block w-full" required value="{{ old('numero_documento', $cliente->numero_documento) }}" />
                        ...
                        <div>
                            ...
                            <x-text-area id="direccion" name="direccion" rows="3" class="mt-1 block w-full">{{ old('direccion', $cliente->direccion) }}</x-text-area>
                    ...
                    @else
                    <p class="text-red-500 dark:text-red-400">No tienes permiso para editar clientes.</p>
                    @endcan
                </div>
            </div>
        </div>
